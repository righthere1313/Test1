...
def main() -> None:
    ...
    with httpx.Client(timeout=180) as client:
        ...
        saved = 0
        for page in range(1, mp4_total_pages + 1):
            url = f"{base_url}{mp4_pages_base_url}/{page}.mp4"
            out_path = pages_out_dir / f"page_{page}.mp4"
        ...
        print("[+] saved mp4 pages:", saved, "/", mp4_total_pages)
        if final_meta:
            print("[+] final mp4 status:", final_meta.get("status"))
