...
def _ensure_local_bg() -> Path:
    p = Path(__file__).resolve().parent / "llm_bg.png"
    if p.exists() and p.stat().st_size > 0:
        return p
    from PIL import Image

    Image.new("RGB", (1600, 900), (245, 245, 245)).save(p)
    return p
...
def _normalize_plan(plan: dict, template_id: str, topic: str, slides_total: int, bg_path: str) -> dict:
    ...
    plan.setdefault("subtitle", "模板学习（客户端直连QA）")
    ...
    while len(slides) < expected:
        slides.append({"title": f"补充{len(slides)+1}", "content": ["要点1", "要点2", "要点3", "要点4", "要点5"], "notes": "", "layout_index": 1, "background_image": bg_path})
    for s in slides:
...
def main() -> int:
    ...
    prompt = (
        "你是PPT课件编排助手。下面给你一个PPT模板的布局摘要（layout列表、页尺寸等）。\n"
        "请你学习这个模板，并输出一个用于后端渲染PPT的JSON（只输出JSON，不要任何解释文字）。\n"
        ...
        "content每页必须有6条要点，每条尽量1-2句，内容要复杂一些：包含前提、推导要点、一个数值例子、以及一句反思/提问。\n"
    ...
    qa_payload = {
        ...
        "session_id": f"client-template-to-llm-{run_id}",
    }
    qa_resp = client.post("/api/v1/chat/qa", json=qa_payload)
    if qa_resp.status_code != 200:
        raise SystemExit(f"qa failed: {qa_resp.status_code} {qa_resp.text}")
    ...
    if not out:
        out = str(Path(__file__).resolve().parent / f"客户端发模板给LLM-复杂内容-{args.topic}-{run_id}.pptx")
